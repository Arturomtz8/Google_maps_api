...
for x in values:
    ...
    try:
        lat_long[x] = r.json()['results'][0]['geometry']['location']['lat'], \
                      r.json()['results'][0]['geometry']['location']['lng']

        if r.json()['results'][0]['geometry']['location_type'] == 'APPROXIMATE':
            lat_long[x]= 'NOT SO GOOD COORDINATES'
...
df.to_excel('Husqvarna_coordinates_v2.xlsx', encoding="utf_8_sig", index=False)